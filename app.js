...
function initLocations() {
  ...
  if (lastPlace && PRESET_LOCATIONS.includes(lastPlace)) {
...
async function fetchWeather() {
  ...
  if (!("geolocation" in navigator)) {
    tempValue.innerHTML = 'n/a<span class="auto-tag">no location</span>';
    return;
  }
  ...
  navigator.geolocation.getCurrentPosition(
    async (pos) => {
      ...
      try {
        ...
        tempValue.innerHTML = `${Math.round(cw.temperature)}\u00B0C<span class="auto-tag">auto</span>`;
        tempValue.dataset.value = cw.temperature;
        ...
        tempValue.innerHTML = 'n/a<span class="auto-tag">fetch failed</span>';
      }
    },
    () => {
      tempValue.innerHTML = 'n/a<span class="auto-tag">location denied</span>';
    },
...
saveBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    // Firestore's offline queue will still hold onto this write and
    // send it once the connection comes back, so this isn't fatal —
    // but let the person know in case they're worried it's lost.
    setStatus(statusLine, "Saved offline — will sync when back online", "ok");
  } finally {
